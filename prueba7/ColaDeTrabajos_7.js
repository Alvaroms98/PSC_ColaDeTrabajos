...
class ColaDeTrabajos {
	...
	constructor( urlNats, {worker=true, nombreCliente=null} ) {
		...
		//
		// Tiempos máximo de vida de los trabajos realizables (ms)
		//
		this.tiempoMaximo = 2000;
...
*/
	...
	async pedirTrabajo( timeout = 0 ) {
		...
		try {
			// Extraer la antiguedad del mensaje
			const sm = await this.jsManager.streams.getMessage(this.datosStream.name, {seq: m.seq});
    		const fechaMensaje = sm.time;
			const fechaAhora = new Date();
			const antiguedad = fechaAhora - fechaMensaje;

			console.log("*** Antiguedad del trabajo recibido ***");
			console.log(antiguedad);

			// Si el tiempo es superior al tiempo máximo respondemos con un "NO REALIZABLE"
			if (antiguedad > this.tiempoMaximo){
				console.log("*** La antiguedad es superior a la especificada, se responde NO REALIZABLE ***");
				let trabajo = JSON.parse( codificador.decode( m.data )  )
				trabajo.mensajeOriginal = m

				await this.responderATrabajo(trabajo,{
					estado: "OLD",
					respuesta:{
						mensaje: "NO REALIZABLE"
					}
				})
